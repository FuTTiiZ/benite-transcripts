import time, os, json, wave, socket
...
def SendRcon(message):
	if RconPassword:
		...
		sock.send(b'\xFF\xFF\xFF\xFF'+bytes(('rcon {0} benite-transcripts:server:receiveTranscript {1}').format(RconPassword, json.dumps(message)), encoding='ascii'))
		sock.close()
...
def ProcessAudio(userName, userID, filename):
	...
	data = { 'Username': userName, 'Message': whatWasSaid }
	print(data)
	SendRcon(data)
	os.remove(filename)
